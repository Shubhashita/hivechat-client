...
const SignUpForm = ({ buttonClasses, buttonForGFT, toggleSignUpMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Map fullName to 'name' as expected by backend
      const payload = {
      ...
      const response = await axios.post(`${process.env.REACT_APP_SERVER_URL || 'http://localhost:5000'}/user/onboard`, payload);

      if (response.data.success) {
        alert("Registration successful! Please login.");
        if (toggleSignUpMode) toggleSignUpMode();
      }
    } catch (err) {
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3 text-sm">
              ...
              >
                ...
                <a
                  href="#"
                  className="text-[#0c3837] hover:text-[#0c3837] font-medium"
                >
                  Terms of Service
                </a>{" "}
                and{" "}
